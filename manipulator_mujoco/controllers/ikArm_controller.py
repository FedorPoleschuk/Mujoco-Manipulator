...
class IKArm(JointEffortController):
    def __init__(
        ...
        L1 = rtb.RevoluteDH(d=0.0795, a=0., alpha=np.pi/2, qlim=[-np.pi/2,np.pi] ) #shoulder roll
        ...
        super().__init__(physics, joints,ojoints, min_effort, max_effort, servo_map, simulation=False)
    ...
    def run(self, target, walk):
        ...
        if not self.walk:
            # print('on')
            sol =  self.robot.ikine_LM(target_pose, q0=p+self.zeros, mask=[1,1,1,0,0,0], slimit=90, ilimit=3, joint_limits=True, tol=0.00001)
            
            # print(sol)
            # print('ommm')
            if sol.success and  sol.q[1]>0:
                print('sdsds')
                u =sol.q-self.zeros
                sr = u[0]
                sp= u[1]
                ...
                ay = u[4]
                eff = [sp,sr,ep,ey,ay]
                gain = [(a + b) / 2 for a, b in zip(self.pos, eff)]
                super().run(gain)
